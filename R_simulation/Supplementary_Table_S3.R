...
## Bivariate-random effect analogue of Model S.1 (Poisson zero-inflated mixture)
## under the condition in (S.2), with c1 = c2 = 0 and d1 = d2 = 2.
##
## Y_t = Z_t * N_t,
##   Z_t | Theta ~ Ber(logistic(c + Theta1)),
##   N_t | Theta ~ Pois(softplus(d + Theta2)),
...
simulate_once <- function(sigma1_sq, sigma2_sq = 1,
  ...
  activation_probability <- sigma_fun(theta1)
  poisson_mean <- softplus_fun(d + theta2)
  
  # Y1 = Z1 * N1
  z1 <- rbinom(N, size = 1, prob = activation_probability)
  n1 <- rpois(N, lambda = poisson_mean)
  y1 <- z1 * n1
  
  # Generate Y2 independently of Y1 conditional on the random effects
  z2 <- rbinom(N, size = 1, prob = activation_probability)
